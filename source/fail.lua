...
function fail:init(...)
    ...
    assets = { -- All assets go here. Images, sounds, fonts, etc.
        image_fail_bad = gfx.image.new('images/fail_bad'),
        image_fail_good = gfx.image.new('images/fail_good'),
    ...
    gfx.sprite.setBackgroundDrawingCallback(function(x, y, width, height) -- Background drawing
        if vars.showtime then
            if vars.score >= 10 then
                assets.image_fail_good:draw(0, 0)
            else
                assets.image_fail_bad:draw(0, 0)
            end
        end
